...
class Event:
    ...
    def check_triggers(player, partner, ui):
        ...
        if player.state["体感"] <= 30:
            ui.console.print(f"\n你感觉到疼痛，哪里都在痛，尤其是心脏。今天埃里给你注射了药，但这药似乎越来越不管用了。操他妈的。", style="bold red")
        elif player.state["体感"] <= 50:
            ui.console.print(f"\n肌肤相亲的触感缓解了你的疼痛，总算是好受点了。",style="yellow")
        elif player.state["体感"] <= 80:
            ui.console.print(f"\n兴奋的感觉渐渐盖过了痛苦，你渴求着更多亲密的接触。",style="blue")

        if player.state["精神"] <= 25:
            ui.console.print(f"\n{player.name}感觉糟透了，一种熟悉的焦躁在{player.name}身体里蔓延，有什么东西快要溢出来，总之不会是什么好东西。\n{partner.name}正在看着{player.name}，目光里透露着担忧……有什么好担忧的？看了就让人烦躁。", style = "bold red blink")
